        ...
        {NavLinks.map((link, index) => {
          if (link.dropdownItems) {
            return (
              ...
              >
                {link.dropdownItems.map((item, subIndex) => (
                  <NavDropdown.Item
                    key={subIndex}
                    className={`  ${link.route === pathname && "bg-primary rounded-4 text-white  p-2"} dropdown_link`}
                  >
                    <Link
            ...
            return (
              <Link
                key={index}
                className= {` ${link.route === pathname && "bg-primary rounded-4 text-white  p-2"} text-dark text-capitalize nav__link`}
                href={link.route}
              >
                {link.label}
              </Link>
...
const AnimatedNavbar = ({ children, ...rest } : {children : React.ReactNode} ) => {
